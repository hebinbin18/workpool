...
void CSmtp::Send()
{
	...
	// EHLO <SP> <domain> <CRLF>
	sprintf(SendBuf,"EHLO %s\r\n",GetLocalHostName()!=NULL&&strlen(GetLocalHostName()) ? m_sLocalHostName.c_str() : "domain");
	SendData();
	bAccepted = false;
	do
	{
...
{
	printf("SMTP Server replay is [%s]\n", RecvBuf);
	assert(RecvBuf);
	if(RecvBuf == NULL)
		return 0;
...
void CSmtp::SendData()
{
	...
	assert(SendBuf);
	printf("SMTP Client send data is [%s]\n", SendBuf);
	if(SendBuf == NULL)
...
int mailTest()
{
    ...
    try
	{
		...
		mail.SetSMTPServer("smtp.exmail.qq.com", 25);
		mail.SetLogin("hebin@zouke.com");
		mail.SetPassword("zk123456");
  		mail.SetSenderName("hebinbin");
  		mail.SetSenderMail("hebin@zouke.com");
  		mail.SetReplyTo("hebin@zouke.com");
  		mail.SetSubject("The message");
  		mail.AddRecipient("43171947@qq.com");
